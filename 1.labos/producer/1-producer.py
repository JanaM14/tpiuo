...
async def run():
    ...
    client_auth = requests.auth.HTTPBasicAuth('6dh64eDiNOG4dG06LEYfLQ', 'i56BzqkJOQiReIW4wezh20HcrXTB7w')
    post_data = {"grant_type": "password", "username": "jmm123478", "password": "severina123"}
    headers = {"User-Agent": "ChangeMeClient/0.1 by YourUsername"}
    response = requests.post("https://www.reddit.com/api/v1/access_token", auth=client_auth, data=post_data, headers=headers)
    var = response.json()
    headers = {"Authorization": f'bearer {var["access_token"]}', "User-Agent": "ChangeMeClient/0.1 by YourUsername"}
